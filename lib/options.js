...
schema.forEach(function (option) {
    ...
    minischema.default[option.long] = value != null ? value : null;
...
function parseConfig(flags, cache) {
    ...
    try {
        ...
        throw new Error(
            'Cannot parse `' + flags + '` as JSON: ' + message
        );
    }
 ...
 * Parse `plugins`.
 *
 * @param {*} value - Raw plugins.
 * @return {Object} - Parsed plugins.
 */
...
function options(flags, configuration) {
    ...
    config = toCamelCase(minimist(flags, minischema));

    schema.forEach(function (option) {
        if (option.type === 'string' && config[option.long] === '') {
            throw new Error(
                'Missing value:' + inspect(option).join(' ')
            );
        }
    });
